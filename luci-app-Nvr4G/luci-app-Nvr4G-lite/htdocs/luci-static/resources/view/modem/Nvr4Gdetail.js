...
modemDialog: baseclass.extend({
		...
		handleSave: function(ev) {
			...
			return uci.load('Nvr4G').then(function() {

				var vx = document.getElementById('mselect').value; 

				uci.set('Nvr4G', '@Nvr4G[0]', 'device', vx.toString());
	...
	load: function() {
		return L.resolveDefault(fs.exec_direct('/usr/share/Nvr4G-lite/Nvr4G.sh', [ 'json' ]));
	},
	...
	render: function(data) {
		...
		var json = JSON.parse(data);
			...
			pollData: poll.add(function() {
				return L.resolveDefault(fs.exec_direct('/usr/share/Nvr4G-lite/Nvr4G.sh', 'json'))
					...
					var p = (json.signal);
					if (p < 0)
						icon = L.resource('icons/Nvr4G-0.png');
					else if (p == 0)
						icon = L.resource('icons/Nvr4G-0.png');
					else if (p < 20)
						icon = L.resource('icons/Nvr4G-0-20.png');
					else if (p < 40)
						icon = L.resource('icons/Nvr4G-20-40.png');
					else if (p < 60)
						icon = L.resource('icons/Nvr4G-40-60.png');
					else if (p < 80)
						icon = L.resource('icons/Nvr4G-60-80.png');
					else
						icon = L.resource('icons/Nvr4G-80-100.png');
		...
		var info = '';
		m = new form.JSONMap(this.formdata, _(''), info);

		s = m.section(form.TypedSection, 'Nvr4G', '', null);
		s.anonymous = true;
